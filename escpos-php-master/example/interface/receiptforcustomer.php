...
//       $discounted_price = $price - $discounted_amount;
    ...
    $printer -> feed();
    $printer -> text("Customer Rcpt #: ".$cust_id);
    $printer -> feed();
    $printer -> feed();
...
class item
{
    ...
    public function __toString()
    {
        $left_name = str_pad($this -> name, 12, ' ', STR_PAD_RIGHT);
        $left_price = str_pad($this -> price, 7, ' ', STR_PAD_RIGHT);
        $center = str_pad($this -> quantity, 4);
        $right = str_pad($this -> discounted_amount, 6, ' ', STR_PAD_LEFT);
